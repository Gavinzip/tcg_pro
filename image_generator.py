...
async def generate_report(card_data, snkr_records, pc_records, out_dir=None):
    ...
    if is_raw:
        ...
        # Only count S/PSA10 to match S (PSA 10) Trend chart
        ...
        all_dates = []
        for r in pc_records + snkr_records:
            all_dates.append(_parse_d(r['date']))
        ...
        stat_2_t, stat_2_v = f"{target_grade} Min (最低)", f"${min(tgt_prices):.2f}" if len(tgt_prices) > 0 else "N/A"
        stat_3_t, stat_3_v = f"{target_grade} Max (最高)", f"${max(tgt_prices):.2f}" if len(tgt_prices) > 0 else "N/A"
        ...
        all_dates = []
        for r in pc_records + snkr_target_records:
            all_dates.append(_parse_d(r['date']))
